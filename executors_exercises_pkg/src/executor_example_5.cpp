...
/*
You see that ONLY the FIRST CALLBACK is executed.
The reason that only one Callback is executed is that the Executor only created
one thread per Node. So you only have one thread to execute everything;
therefore, you cannot execute several Callbacks in parallel. And the reason why
the timer_callback_1 is executed is that it has been instantiated first in the
constructor.
*/