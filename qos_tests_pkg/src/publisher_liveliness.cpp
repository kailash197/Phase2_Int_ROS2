...
public:
  PublisherQoS(int &argc, char **argv) : Node("publisher_qos_obj") {
    ...
    group_events_clb = this->create_callback_group(
        rclcpp::CallbackGroupType::MutuallyExclusive);
    /*
    Because you only execute two regular Callbacks (timer_callback and
    alive_callback), if you were to use RentrantCallbackGroups, the system would
    execute multiple times simultaneously timer_callback or alive_callback. You
    do not want that. You want those Callbacks only to be executed one instance
    of each. That is why you use mutually exclusive.
    */
...
int main(int argc, char *argv[]) {
  ...
  rclcpp::executors::MultiThreadedExecutor executor(rclcpp::ExecutorOptions(),
                                                    3);
  // need at least three threads to process three Callbacks in parallel.
